// Highlights pack — the 9:16 frame that lives inside a Highlight.
//
// Copied from 04-community/scenes/stories.js and trimmed. The geometry is
...
// (`steps`, `receipt`, `checks`) are also the ones that take no sticker. A frame
// where every element asks for a tap stops feeling like a sequence and starts
// feeling like a survey.
//
// ── What this pack dropped ─────────────────────────────────────────────────
//
// `leaf`, `seam` and `frmark` are not here. The bilingual furniture — the intro
// frame with the progress replica, and the FR card that marks the seam — is
// rendered ONCE, in 04-community/00-language-notice/, and reused at the front
// and middle of every tray in this pack. Two copies of the same signpost in two
// packs is how a signpost starts pointing two different ways.
