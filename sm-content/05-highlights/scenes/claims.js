// Highlights pack — the claim rules.
//
...
//
// Inherited from 04-community unchanged, because this pack has the same
// permission and the same price for it:
//
//   A frame or caption that names a retailer MUST carry the non-affiliation
//   line, on the art, in the same language.
//
// Enforced structurally in verify.js (checkDisclaimerPairing), not by a regex,
// because it is a pairing rule and a regex cannot see a pair. The app's
// legal/MARKETING_CLAIMS.md names the failure mode: "the slogan gets clipped
// into a banner / social-media-card / marketing-email without the fine print."
// A caption disclaimer survives until the first screenshot. A rendered one does
// not have that failure mode.
//
// ── The rule that matters most in THIS pack ────────────────────────────────
//
// This is the pack that talks about money — credits, packs, the subscription —
// and the currency and percentage bans are therefore doing more work here than
// anywhere else. They are not squeamishness. A price printed into a PNG outlives
// the price it prints, and the evidence is in the app's own repo: README.md
// still advertises a $12/month subscription that has not cost that in months,
// and pricing.config.js still carries a break-even string computed against it.
// Both are text somebody has to remember to update. A rendered frame is worse,
// because nobody re-reads a PNG.
//
// So the Unlimited argument is made on SHAPE — unlimited scans, no per-drop
// charge, one flat fee, two months free on the year — and the number lives in
// the app, where it is always current by construction.
//
