// Community pack — layout helpers shared by the post, story and cover scenes.
//
// Copied from evergreen/scenes/layout.js (each pack owns its own renderer, so a
// tweak in one can never re-render another's PNGs) with one addition:
// hookBlock() now accepts align:"middle", which the bilingual divider slide
// needs and no evergreen frame did.
//
 ...
 */
async function hookBlock(lines, { x, y, colW, token = "hook", min = 44, maxH = Infinity, op = FADE.full, ops = null, fill = C.inkOnField, align = "start" }) {
  const t = TYPE[token];
  ...
  // "middle" exists for the divider slide only. text-anchor alone is not
  // enough: the anchor POINT has to move to the column centre too, or a centred
  // string is centred on the left margin.
  const anchorX = align === "end" ? x + colW : align === "middle" ? x + colW / 2 : x;
  const svg = lines
    .map((s, i) =>
      text(s, {
        x: anchorX,
        ...
        tracking: t.tracking,
        family: t.family,
        // `ops` gives a per-line opacity, and it exists for the bilingual
        // frames: a hook whose whole subject is that this account is in two
        // languages cannot itself be in one. The French line is set full
        // weight and the English sits under it dimmed, at the SAME SIZE --
        // dimming is a reading-order signal, shrinking would be a status one.
        // Same rule subBlock() already applies to the line under a hook.
        op: ops && ops[i] != null ? ops[i] : op,
        fill,
        anchor: align,
      })
    )
...
function subBlock(lines, { x, y, colW, op = 0.68, ops = null, fill = C.inkOnField, size = TYPE.sub.size }) {
  ...
  const svg = lines
    .map((s, i) =>
      text(s, {
        ...
        family: TYPE.sub.family,
        // `ops` gives a per-line opacity, and it exists for exactly one thing:
        // the bilingual frames set their French line brighter than the English
        // line under it. Two lines at equal weight read as one paragraph that
        // happens to be in two languages; one bright and one dimmed reads as a
        // line and its translation. Size stays equal on purpose - dimming is a
        // reading-order signal, shrinking would be a status one.
        op: ops && ops[i] != null ? ops[i] : op,
        fill,
      })
    )
